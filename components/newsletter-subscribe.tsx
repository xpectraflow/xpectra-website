...
export const NewsletterSubscribe = () => {
  ...
  return (
    ...
    >
      {/* Background Glows */}
      <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 w-96 h-96 bg-white/[0.02] blur-[100px] rounded-full pointer-events-none" />
      <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/4 w-64 h-64 bg-white/[0.01] blur-[80px] rounded-full pointer-events-none" />
      
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
        <div className="flex-1 text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-mono text-white/50 uppercase tracking-wider mb-6">
            ...
            <form action={formAction} className="space-y-4">
              <div className="relative">
                <Input
                  ...
                  required
                  className="bg-white/5 border-white/10 focus:border-white/30 h-14 pl-5 text-white text-lg rounded-xl"
                  disabled={isPending}
                />
                {state?.error && (
                  <motion.div 
